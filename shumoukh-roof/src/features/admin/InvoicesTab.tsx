...
const STATUS_LABELS: Record<string, string> = { draft: "مسودة", pending: "قيد الانتظار", paid: "مدفوعة" };
...
export default function InvoicesTab() {
  ...
  const handleDelete = async (inv: any) => {
    if (!window.confirm(`هل أنت متأكد من حذف فاتورة "${inv.client}"؟`)) return;
    try {
    ...
    <div className="space-y-4">
      <div className="glass-card p-4">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-ink-muted">فاتورة</span>
          </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="glass-card p-3 text-center">
          <p className="text-xs text-ink-muted font-bold">مدفوعة</p>
          <p className="text-lg font-black text-emerald-600">{totalPaid.toFixed(0)} د.أ</p>
        </div>
        <div className="glass-card p-3 text-center">
          <p className="text-xs text-ink-muted font-bold">مستحقة</p>
          <p className="text-lg font-black text-amber-600">{totalPending.toFixed(0)} د.أ</p>
        </div>
        <div className="glass-card p-3 text-center">
          <p className="text-xs text-ink-muted font-bold">مسودة</p>
          <p className="text-lg font-black text-slate-600">{totalDraft.toFixed(0)} د.أ</p>
        </div>
      </div>
      ...
      {loading ? (
          ...
          {filtered.map((inv) => (
            <div key={inv.id} className="glass-card p-4">
              ...
              <div className="flex items-center justify-between mb-3">
                <span className="text-lg font-black text-ink-primary">{inv.amount?.toFixed(0)} د.أ</span>
                <span className="text-[10px] text-ink-muted">{new Date(inv.createdAt).toLocaleDateString("ar-JO")}</span>
              </div>
              <div className="flex items-center gap-1">
                {NEXT_STATUS[inv.status] && (
                  <button onClick={() => handleStatusChange(inv, NEXT_STATUS[inv.status])}
                    ...
                    {NEXT_STATUS[inv.status] === "pending" ? "تعليق" : "دفع"}
                  </button>
                )}
        ...
        <div className="text-center py-12 text-ink-muted">
          ...
          <p className="text-sm font-black">لا توجد فواتير</p>
        </div>
      )}
    </div>
  );
